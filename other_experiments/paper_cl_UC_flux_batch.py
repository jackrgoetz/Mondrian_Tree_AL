...
for n_final_ind, n_final in enumerate(n_finals):
    ...
    for data_seed in data_seeds:
        ...
        for tree_seed in tree_seeds:
            ...
            new_labelled_points = []
            for i in range(int((n_final - n_start)/batch_size)):
                MT_al.al_calculate_leaf_proportions()
                MT_al.al_calculate_point_probabilities_adjustment(n_final)
                probs = [x if x is not None else 0 for x in MT_al._al_point_weights_adjustment]
                temp_new = np.random.choice(range(n), batch_size, p=probs,replace=False)
                new_labelled_points += list(temp_new)
                for point in temp_new:
                    MT_al.label_point(point, y[point])
...
plt.tight_layout()
plt.savefig('graphs/sim_batch_uc_' + 
